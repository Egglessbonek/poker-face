...
/** Keyword -> evidence signal, in priority order. Only signals that exist in `fuse.ts` can come out. */
const SIGNAL_WORDS: Array<[RegExp, string]> = [
  [/blink/, "blink_rate"],
  [/\bstill|froze|frozen|motionless|stiff/, "freeze"],
  [/tension|tense|jaw|brow|clench/, "tension"],
  [/smile|smirk|grin/, "smile_leak"],
  [/chip/, "chip_glance"],
...
export function canonicalTells(raw: string[], opponents: OpponentView[]): string[] {
  const humans = opponents.filter((o) => !o.folded && o.kind === "human" && o.tells && o.tells.evidence.length + 1 > 0);
  const out: string[] = [];
  ...
  for (const r of raw) {
    ...
    let matched = false;
    for (const [re, signal] of SIGNAL_WORDS) {
      if (!re.test(lower)) continue;
      for (const o of pool) {
        const e = o.tells!.evidence.find((ev) => ev.signal === signal);
        if (e) { push(`${o.name}: ${e.text}`); matched = true; break; }
      }
      if (matched) break;
    }
    if (!matched && /bluff|arousal|confiden|read|likel/.test(lower) && pool.length === 1) {
